...
| 버전 | 수정일 | 변경 요약 |
| --- | --- | --- |
| v0.7 | 2026-07-14 | Phase 3 Checklist v1 완료 반영(P3-00~P3-07). §2 스냅샷을 Phase 3 완료·OS MVP 컷라인 도달·다음 단계 P3-08로 갱신, §3 checklist 모듈 문서 [x], §7 Phase 3 [x] + 완료 요약. weekly/monthly·알림·통계·첨부는 후순위 |
...
```text
갱신일: 2026-07-14
단계: Phase 1·1.5 완료 · Phase 2 Notice v1 완료 · Phase 3 Checklist v1 완료 → OS MVP 컷라인 도달
브랜치: main 기준 P1-01~P1-08 + Phase 1.5 + Phase 2(P2-00~P2-07) + Phase 3(P3-00~P3-07) 반영 완료
코드: OS 셸 + Inventory(재고관리) + Notice v1(공지사항) + Checklist v1(오픈/마감 체크리스트) 사용 가능
Checklist v1: 부서 daily 정기업무 — 오늘 화면(내 부서), 수행자 완료/취소(멱등·재활성), completed_by/at,
              TEAM_LEADER 이상 누락 현황(TEAM_LEADER 본인 부서 / MANAGER·ADMIN 전체 활성 부서),
...
[x] docs/modules/checklist/CHECKLIST_*.md (Phase 3 생성: PRODUCT_SPEC / TECH_SPEC / TASKS)
```
...
[x] P3-01    checklist 앱 뼈대 + /checklists/ placeholder → checklist 앱 전환
[x] P3-02    3모델(ChecklistItem/DepartmentChecklistItem/ChecklistRecord) + checklist.0001_initial (리허설 DB 적용)
[x] P3-03    오늘의 체크리스트 조회 (내 부서 daily selector, KST localdate)
