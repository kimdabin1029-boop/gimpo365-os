...
### 15.6 [검토 필요] OperationalBaseModel 미구현

```text
OS_TECH_SPEC §17 은 core 공통 abstract base model(OperationalBaseModel 등)을 제안하지만,
현재 core/models.py 에는 Department 만 있고 base model 은 아직 구현되어 있지 않다.
→ Notice 는 OperationalBaseModel 상속을 기본 방향으로 하며,
   base model 이 없으면 Notice 구현 전에 먼저 신설한다(P2-01.5, migration 없음 — TECH_SPEC §10).
```
...
- 선행 작업: 15.6 OperationalBaseModel 신설(P2-01.5).
```
