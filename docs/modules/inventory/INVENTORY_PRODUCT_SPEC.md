...
Item
- 전역 품목 마스터
- 주문·재고관리 단위(unit)를 소유한다 (P3-07.6). 규격·포장은 품목명에 포함(별도 규격 필드 없음).

ManagedItem
- 특정 부서가 관리하는 품목
- Item + Department 조합
- 단위는 갖지 않는다(Item.unit 사용). 부서마다 단위가 달라야 하면 별도 Item 으로 정의.
```

재고 거래는 `Item`이 아니라 `ManagedItem`에 연결한다.
단위(unit)는 `Item`이 소유하며, 같은 Item 을 여러 부서에서 관리해도 단위는 동일하다. (P3-07.6)
