...
## 17. 기존 OS 문서와의 정합성 (검토 필요 항목)

P3-00 원칙: 기존 문서를 임의 수정하지 않고 아래에 "[검토 필요]"로 기록한다.

### 16.1 [검토 필요] "당일 마감담당자" 표현

```text
충돌 문서: OS_PRODUCT_SPEC §5.3, OS_TECH_SPEC §7·§25, OS_ARCHITECTURE §14
충돌 표현: "책임 확인 단위 = 당일 마감담당자"
이번 Checklist 결정: 사전 담당자 지정 없이 실제 수행자(completed_by)만 기록한다.
                    '마감담당자'는 사전 배정이 아니라 '그날 실제로 완료한 사람'으로 해석한다.
향후 수정 필요: 개념은 정합(완료자 = 그날 수행자). 문서 표현을 "사전 지정 아님"으로 명확화하면 좋음(경미).
```
...
```text
- 하드 충돌: 없음.
- 경미 표현 정리 후보: 16.1 "당일 마감담당자" → "사전 지정 아님" 명확화(선택).
```
